...
Downloads (public, no auth required):
  - MIDs 5.0 (dyadic Militarized Interstate Disputes)
  - Inter-State War data (v4.0)
  - National Material Capabilities (CINC scores) v6.0

Source: https://correlatesofwar.org/data-sets/

Each dataset ships as a zip containing a CSV. This script downloads (if not
already present in data/cow/), extracts, and loads normalized rows into the
`cow_mids`, `cow_wars`, `cow_capabilities` tables in the app's SQLite file —
the same file used for the case cache (see app/cache/).
"""
...
DATASETS = {
    "mids": {
        ...
        "csv_hint": "MIDB",  # dyadic/participant-level file within the zip
    },
    "wars": {
    ...
    "capabilities": {
        "url": "https://correlatesofwar.org/wp-content/uploads/NMC-60-abridged.zip",
        "csv_hint": "NMC",
    },
}
...
def load_mids(conn: sqlite3.Connection, csv_text: str) -> int:
    ...
    for row in reader:
        try:
            rows.append(
                (
                    int(row.get("DispNum3", row.get("dispnum", 0)) or 0),
                    (row.get("StateAbb") or row.get("stateabb") or "").strip(),
                    "",
                    int(row.get("StYear", row.get("styear", 0)) or 0),
                    _int_or_none(row.get("EndYear", row.get("endyear"))),
                    _int_or_none(row.get("HostLev", row.get("hostlev"))),
                    _int_or_none(row.get("Outcome", row.get("outcome"))),
                )
            )
...
def load_wars(conn: sqlite3.Connection, csv_text: str) -> int:
    ...
    for row in reader:
        try:
            rows.append(
                (
                    int(row.get("WarNum", row.get("warnum", 0)) or 0),
                    (row.get("WarName", row.get("warname")) or "").strip(),
                    int(row.get("StartYear1", row.get("startyear1", 0)) or 0),
                    _int_or_none(row.get("EndYear1", row.get("endyear1"))),
                    _int_or_none(row.get("Outcome", row.get("outcome"))),
                    None,
                )
            )
...
def load_capabilities(conn: sqlite3.Connection, csv_text: str) -> int:
    ...
    for row in reader:
        try:
            rows.append(
                (
                    (row.get("StateNme", row.get("statename")) or "").strip(),
                    int(row.get("Year", row.get("year", 0)) or 0),
                    _float_or_none(row.get("CINC", row.get("cinc"))),
                )
            )
        except (ValueError, TypeError):
            continue
